...
* Title: Effect of TAVR adoption on cardiologist provision of preventive care: 
	4b -- how likeliy is it to use aspirin? 
...
*** Outcome: aspiring use 
gen aspirin = inlist(hcpcs_cd, "G8958", "G8959", "G8895", "G9277", "G9435", "G9793", "4086F", "M1055")
gen statin = inlist(hcpcs_cd, "G8816", "G9441", "G9507", "G9664", "G9796") 
collapse (max) aspirin statin, by(bene_id npi year) fast
collapse (mean) aspirin statin, by(npi year) fast 
...
replace aspirin = 0 if _fillin == 1
replace statin = 0 if _fillin == 1
...
foreach y of local outcomes { 
	...
	*** Graph results
	...
	graph save "$mydir/ToExport_EventStudy_Likelihood_`m'.gph", replace
	graph export "$mydir/ToExport_EventStudy_Likelihood_`m'.pdf", as(pdf) replace
}
