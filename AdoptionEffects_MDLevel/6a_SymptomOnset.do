...
* Title: Effect of TAVR adoption on cardiologist provision of preventive care: 
	6a -- how likeliy is symptomatic AS to be reported/diagnosed? 
...
*** Outcome: fraction of patients with *symptomatic* AS*
gen symptoms = 0
replace symptoms = 1 if inlist(line_icd_dgns_cd, "R55", "R570", "7802", "78551") // syncope
replace symptoms = 1 if substr(line_icd_dgns_cd, 1, 3) == "I20" | ///
	inlist(line_icd_dgns_cd, "4111", "4131", "4139") // angina
replace symptoms = 1 if inlist(substr(line_icd_dgns_cd, 1, 3), "I50", "428") // heart failre
collapse (max) symptoms, by(bene_id npi year) fast
collapse (mean) symptoms, by(npi year) fast 
...
replace symptoms = 0 if _fillin == 1
...
foreach y of local outcomes { 
	...
	*** Graph results
	...
	graph save "$mydir/ToExport_EventStudy_FracSymptomatic.gph", replace
	graph export "$mydir/ToExport_EventStudy_FracSymptomatic.pdf", as(pdf) replace
}
