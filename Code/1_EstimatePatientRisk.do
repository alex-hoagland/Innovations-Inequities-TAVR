...
forvalues i = 2010/2017 { 
	di "YEAR: `i'"
	use "$datadir/PatientRisk.dta" if file_year == `i', clear
...
use "$datadir/PatientRisk_2010.dta"
...
gen year = riskvar_year
merge m:1 fips_string year using "$mydir/2_Data/Income/fips_income.dta", keep(1 3) 
	// 22% are missing county assignment -- need to fix this in order to get all income!
drop _merge year
...
* Demographics: sex, age, race (black, hispanic, white, other), income quintile, # of chronic conditions, time trend (year FE)
...
drop sex race file_year 
bysort bene_id: ereplace death_dt = min(death_dt) 

xtile income_quintile = medinc_65plus, nq(5)
forvalues i = 1/4 { 
	gen riskvar_incomequintile_`i' = (income_quintile == `i')
}

gen riskvar_numccs = 0
foreach v of var ami alzh atrialfb cataract chrnkidn copd chf diabetes glaucoma ///
	hipfrac ischmcht depressn osteoprs ra_oa strketia cncrbrst cncrclrc cncrprst cncrlung ///
	cncrendm anemia asthma hyperl hyperp hypert hypoth { 
	replace `v' = 3 if `v' == 1 // count meeting claims criteria as sufficient
	replace riskvar_numccs = riskvar_numccs + 1 if `v' == 3
}

* General chronic conditions : 
// CHF, diabetes, dialysis, hypertension, stroke, AMI, COPD
gen riskvar_chf = (chf == 3) 
gen riskvar_diabetes = (diabetes == 3) 
gen riskvar_hypert = (hypert == 3) 
gen riskvar_stroke = (strketia == 3) 
gen riskvar_ami = (ami == 3) 
gen riskvar_copd = (copd == 3) 
drop ami* alzh* atrialfb* cataract* chrnkidn* copd* chf* diabetes* glaucoma* ///
	hipfrac* ischmcht* depressn* osteoprs* ra_oa* strketia* cncrbrst* cncrclrc* ///
	cncrprst* cncrlung* cncrendm* anemia* asthma* hyperl* hyperp* hypert* hypoth*
drop atrialfm-cncendme
...
sort bene_id riskvar_year
order bene_id riskvar_year riskvar_* 
compress
save "$datadir/PatientRisk.dta", replace
********************************************************************************
...
use "$datadir/all_InpatientCardiology.dta" if substr(icd_prcdr_cd1,1,1) == "0", clear
keep bene_id file_year from_dt icd_prcdr_cd1
duplicates drop
rename file_year riskvar_year
merge m:1 bene_id riskvar_year using "$datadir/PatientRisk.dta", keep(3) nogenerate

* Prior surgeries (also from claims): 
// any prior surgery, # of prior sugeries, preevious coronary artery bypass, 
// previous valve surgery, previous PCI, 
gen surgery = 1 
bysort bene_id (from_dt): gen riskvar_numsurgeries = sum(surgery) - 1
gen riskvar_anysurgery = (riskvar_numsurgeries > 0)
gen riskvar_previousbypass = 1 if substr(icd_prcdr_cd1,1,3) == "021"
gen riskvar_previousvalve = 1 if inlist(substr(icd_prcdr_cd,1,4), "02RF", "02RG", "02RH", "02RJ")
gen riskvar_previouspci = 1 if substr(icd_prcdr_cd,1,3) == "027" 
bysort bene_id (from_dt): carryforward riskvar_previous*, replace // move into future
foreach v of var riskvar_previous* { 
	replace `v' = 0 if missing(`v')
}

// Merge these into original data in order to use as out-of-sample predictors
preserve
collapse (max) riskvar_anysurgery riskvar_numsurgeries riskvar_previous*, by(bene_id riskvar_year) fast
merge 1:1 bene_id riskvar_year using "$datadir/PatientRisk", keep(2 3) nogenerate
foreach v of var riskvar_anysurgery riskvar_numsurgeries riskvar_previous* {
	replace `v' = 0 if missing(`v')
}
save "$datadir/PatientRisk.dta", replace
restore
...
order bene_id from_dt death_dt
gen outcome_mortality = (!missing(death_dt) & death_dt-from_dt <= 60)
logit outcome_mortality riskvar_* i.riskvar_year

// Save regression table for appendix
estimates store predrisk
esttab using "$output/Appendix_PredictedRisk_TableFrag.tex", label nostar frag  

* Predict out-of-sample
use "$datadir/PatientRisk.dta", clear
cap drop predicted_risk 
predict predicted_risk, pr
save "$datadir/PatientRisk.dta", replace
********************************************************************************
...
sum predicted_risk, d
hist predicted_risk, percent graphregion(color(white)) fcolor(ebblue%30) lcolor(ebblue) ///
	xtitle("Predicted Pr(60-day Surgical Mortality)") ytitle("%",angle(horizontal)) ///
	ylab(,angle(horizontal)) xsc(r(0(.2)1)) xlab(0(.2)1) ///
	xline(.03, lpattern(dash) lcolor(red)) xline(.08, lpattern(dash) lcolor(red))
graph save "$mydir/ToExport/PredictedRisk_Histogram.gph", replace
graph export "$mydir/ToExport/PredictedRisk_Histogram.pdf", as(pdf) replace

// Make summary table of covariates / risk as function of if patient is in a TAVR-adopting county or not
do "$mydir/3_SourceCode/5_CrowdOut_Decomposition/1a_PatientSummaryTable.do"
********************************************************************************
