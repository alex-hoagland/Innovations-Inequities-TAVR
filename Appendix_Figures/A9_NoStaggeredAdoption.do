/*******************************************************************************
* Title: ID Effect of TAVR Adoption on Other Surgery Outcomes: NO STAGGERED ADOPTION
* Created by: Alex Hoagland
* Created on: 7/30/2024
* Last modified on: 
* Last modified by: 
* Purpose: Assess whether TAVR crowded out volumes of other procs 
	- does not utilize any staggered adoption timing in identification, just relative to 2011q4
* Notes: 
...
// if ever adopted, look at 2012q1 as adoption date (2011q4 as reference group)
replace t_adopt = 206 if !missing(t_adopt)
...
set scheme cblind1
cap graph drop * 
lpdid outcome, unit(CZID) time(yq) treat(treatdummy) pre_window(4) post_window(12) pmd(4)
...
addplot: , xtitle("Quarters Around TAVR Adoption") ytitle("") xsc(r(-4(4)12)) xlab(-4(4)12) ///
	text(3 4 "Pre-treatment median: `pretreat'", place(e)) ///
	text(`pooledfx' -2 "Pooled Effect: `pooledfx'***", place(s)) 
	
// Save graphs
local figname = "Robustness_NOStaggeredAdoption_`1'Treatments_`2'Margin"
